...
process.stdin.on( "data", function( input_from_user )
{
   ...
   {
	   process.stdout.write( " " + name_from_keyboard.length ) ;
	   
   }
      process.stdout.write( "\n\n Here is your name in reverse order : \n\n ");
	  
	var alo = name_from_keyboard.length ;
	
	while ( alo > 0 ) 
	{
		alo --;
		process.stdout.write ( name_from_keyboard[alo]);
	}
   ...
   process.stdout.write ( "\n\n Your name in morse codes is : \n\n");
    for ( let alo  =  0 ;
             alo  <  name_from_keyboard.length ;
             alo  ++ )
   {
      let index_of_character_in_array  =
             array_of_morse_codes.indexOf( name_from_keyboard[ alo ] ) ;
